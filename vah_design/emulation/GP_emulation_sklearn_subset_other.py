...
sd_list = ['sd_'+obs for obs in exp_label]
y_train_noise = y_er_train[sd_list]
...
# obs_list = ['pT_fluct_[0 5]', 'pT_fluct_[ 5 10]', 'pT_fluct_[10 15]',
#        'pT_fluct_[15 20]', 'pT_fluct_[20 25]', 'pT_fluct_[25 30]',
#        'pT_fluct_[30 35]', 'pT_fluct_[35 40]', 'pT_fluct_[40 45]',
#        'pT_fluct_[45 50]', 'pT_fluct_[50 55]', 'pT_fluct_[55 60]', 'v22_[0 5]',
...
#        'pT_fluct_[15 20]', 'pT_fluct_[20 25]', 'pT_fluct_[25 30]',
#        'pT_fluct_[30 35]', 'pT_fluct_[35 40]', 'pT_fluct_[40 45]',
#        'pT_fluct_[45 50]', 'pT_fluct_[50 55]', 'pT_fluct_[55 60]', 'v32_[0 5]', 'v32_[ 5 10]',
#        'v32_[10 20]', 'v32_[20 30]', 'v32_[30 40]', 'v32_[40 50]', 'v42_[0 5]',
#        'v42_[ 5 10]', 'v42_[10 20]', 'v42_[20 30]', 'v42_[30 40]',
#        'v42_[40 50]']

y_train = y_train[obs_list]
y_test = y_test[obs_list]
sd_name = ['sd_'+obs for obs in obs_list]
y_train_noise = y_er_train[sd_name]
print(y_train.shape,y_test.shape,y_train_noise.shape)
...
np.save('INV_MAT_other_high',inverse_tf_matrix)
np.save('SS_MN_other_high',SS.mean_)
np.save('SS_scale_other_high',SS.scale_)
...
if (os.path.exists(EMU)) and (train_emulators==False):
    ...
    for i in range(0,npc):
        ...
        kernel=1*krnl.RBF(length_scale=ptp,length_scale_bounds=np.outer(ptp, (1e-3, 1e3)))+ krnl.WhiteKernel(noise_level=.1, noise_level_bounds=(1e-3, 1))
...
# +
#obs_list = ['v22_[ 0 5]', 'v22_[ 5 10]', 'v22_[10 20]', 'v22_[20 30]', 'v22_[30 40]',
#       'v22_[40 50]', 'v22_[50 60]', 'v22_[60 70]']
# -

sns.set_context('paper',font_scale=2)
for obs in ['v22','v32','v42']:
    st = index[obs][0]
    ed = index[obs][1]
    nrw = int(np.ceil((ed-st)/4))
    fig, axs = plt.subplots(nrows=nrw,ncols= 4, figsize=(20,nrw*8),sharex=True, sharey=True)
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig(f'pcgp/{obs}_other_high.png', dpi=200)
...
for obs in ['v22','v32','v42']:
    ...
    ed = index[obs][1]
    nrw = int(np.ceil((ed-st)/4))
    fig, axs = plt.subplots(nrows=nrw,ncols= 4, figsize=(20,nrw*8),sharex=False, sharey=False)
    for iii,ax in enumerate(axs.flatten()):
        if iii>=ed-st:
            continue;
        ii=st+iii
