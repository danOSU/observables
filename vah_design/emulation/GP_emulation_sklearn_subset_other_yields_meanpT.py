...
y_train = y_train.drop(labels=obs_list, axis=1)
y_test = y_test.drop(labels=obs_list, axis=1)
sd_name = ['sd_'+obs for obs in obs_list]
y_train_noise = y_train_noise.drop(labels=sd_name, axis=1)
...
np.save('INV_MAT_yields_high',inverse_tf_matrix)
np.save('SS_MN_yields_high',SS.mean_)
np.save('SS_scale_yields_high',SS.scale_)
...
index={}
st_index=0
for obs_group in  ['yields','mean_pT','fluct']:
...
obss = obs_groups['yields']+obs_groups['mean_pT']+obs_groups['fluct']
for obs in obss:
    st = index[obs][0]
    ed = index[obs][1]
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig(f'pcgp/{obs}_yields_high.png', dpi=200)
