...
dfh = pd.concat([df_mean_b0, df_mean_b1, feval_add_train]).to_numpy()
dsdh = pd.concat([df_sd_b0, df_sd_b1, sd_add_train]).to_numpy()

averagevar = np.var(dfh, 0)
predsimuvar = np.mean(np.square(dsdh),0)
fig = plt.figure()
plt.scatter(np.arange(0,predsimuvar.shape[0]), predsimuvar / averagevar, alpha=0.5)
plt.show()
